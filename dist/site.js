...
document.getElementById('inquiry').addEventListener('submit',event=>{event.preventDefault();const data=new FormData(event.currentTarget);const subject='Project inquiry: '+data.get('type');const body=`Name: ${data.get('name')}\nEmail: ${data.get('email')}\nProject: ${data.get('type')}\n\n${data.get('brief')}`;window.location.href='mailto:workspaceinnovates@gmail.com?subject='+encodeURIComponent(subject)+'&body='+encodeURIComponent(body);document.getElementById('form-status').textContent='Your email draft is ready to open. Send it from your email app, or contact workspaceinnovates@gmail.com directly.'});
